	...
	onMount(() => {
		...
		function parse(source: string) {
			...
			function count_characters(runs: any) {
				return runs.reduce(
					(acc: any, cur: any) => acc + cur.text.length,
					0,
				);
			}
		...
		function update() {
			...
			if (now(state.triggers.line)) {
				if (state.line < state.lines.length - 1) {
					...
					if (
						state.character <
						state.lines[state.line].input.characters - 1
					) {
		...
		function draw(time: number) {
			...
			if (
				state.line * line_height + margins.top >=
				height - margins.bottom
			) {
				scroll_y =
					state.line * line_height -
					height +
					margins.top +
					margins.bottom;
			}
			...
			for (let i = 0; i <= state.line; i++) {
				...
				for (let run of state.lines[i].input) {
					...
					if (i === state.line) {
						let run_length =
							state.character - (offset - prompt_length) + 1;
						c.fillText(
